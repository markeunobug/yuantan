...
void Set_TxBuf_GRprobe()
{
    ...
    TxBuf[6] = (GRprobe_total_reg>>8)&0xFF;
    TxBuf[7] = (GRprobe_total_reg)&0xFF;
    int cnti=0;
...
void Set_Mem_Manage(unsigned char ID)
{
    switch(ID)
    {
        ...
        case 0x00:
        {
            ...
            TxBuf[7] = (SamplePoint_Log >> 16)&0xFF;
        ...
        case 0x02:
        {
            Read_BlockAddrCnt = Read_Frame/128 + BlockStartAddr;//从BlockStartAddr开始,因为一直擦写前几块频繁使用，有可能导致块错误
