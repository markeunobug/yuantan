...
unsigned char DSP_Ver = 0x58;                                                  //DSP程序版本号
...
unsigned int  MicroFrame_Sum = 90;                                          //小帧的总数
unsigned char Frame_Interval = 5;                                            //帧间隔
unsigned char MicroFrame_Num = 10;                                           //小间隔电阻率采样点数
unsigned int Angle_Diff = 0;                                                //角差/2
unsigned char Sample_gama =0;                                               //伽马每25秒记录一组数据
//延时启动相关配置
...
unsigned char Relative_Phase_Enable = 1;                                    //0为保持原始IQ输出，1为参考通道4相位同步信号
...
float AD4020_Scale_Coefficient = 6291456000;                                    //归一化系数，换算为真实幅度
const unsigned int CMD_SetFreq[5] = {0x0001,0x0002,0x0004,0x0008,0x0010};                   //控制FPGA数据采集，写入的数据命令
unsigned int CMD_SetAmp[5]  = {15,15,15,15,15};                                       //控制FPGA输出幅度
/*************************************************************/
...
volatile unsigned char state_freq=0;                                                       //存放当前工作频率 在帧内占1字节
...
unsigned char shockz;                                                           //z震动 在帧内占1字节
unsigned int  Temp_Lm135;
...
/*************************************以下参数各个频率选择*********************************************/
unsigned char Auto_Zoom_R1A[3]={0x03,0x03,0x03};                                //R1A 4k、10k、40k档位0x01,0x02,0x01
unsigned char Auto_Zoom_R1B[3]={0x03,0x03,0x03};                                //R1B 4k、10k、40k档位0x03,0x02,0x01
unsigned char Auto_Zoom_R1C[3]={0x03,0x03,0x03};                                //R1C 4k、10k、40k档位0x03,0x02,0x01
unsigned char Auto_Zoom_R2A[3]={0x02,0x02,0x02};                                //R2A 10k、40k、100k档位0x01,0x01,0x01
unsigned char Auto_Zoom_R2B[3]={0x03,0x03,0x03};                                //R2B 10k、40k、100k档位0x01,0x01,0x01
unsigned char Auto_Zoom_R2C[3]={0x02,0x02,0x02};                                //R2C 10k、40k、100k档位0x01,0x01,0x01
unsigned char Auto_Zoom_R3U[2]={0x02,0x01};                                     //R3U 100k、400k档位0x02,0x03
unsigned char Auto_Zoom_R3D[2]={0x02,0x01};                                     //R3D 100k、400k档位0x02,0x03
