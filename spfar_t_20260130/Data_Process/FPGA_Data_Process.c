...
//    signed long long temp1=0;
//    signed long long temp2=0;
    ++num_res[Cur_sector];
    ++num_gama[Cur_sector];
    while((Get_GPIO_Value(FPGA_DATA_READY)==1)&&(time_out<(5000/MicroFrame_Num)))
    {
      time_out++;
      DELAY_US(10);
    }
...
//    RS485_SendLongLong(temp1);
//    RS485_SendLongLong(temp2);
...
void SingleAverage_data_gama(unsigned int temp[],unsigned int num[])
{
    ...
    GRprobe_total_reg =GRprobe_total/20;//累计20s
}
