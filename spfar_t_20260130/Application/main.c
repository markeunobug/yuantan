 ...
 //    Flash_Read_SamplePoint();                   //上电时读取帧数，从外部FLASH
  ...
  //      LED_Toggle();
        ...
        switch(state_main)
        {
            case 0:                     //循环测试各个频率
            {
                static unsigned char temp_num = 0;//暂时使用，后续删除

                static unsigned char set_fre = 0;
                //兼容老上位机
                temp_num++;
                temp_num = temp_num%8;
                if(temp_num <= 3)state_set_temp = temp_num;
                else state_set_temp = temp_num%2 + 4;
                //
                set_fre++;
                set_fre = set_fre%4;
//                state_low = set_fre;
                state_low = 0;//固定值，暂时用于
                state_high = set_fre%2;

                Mode_Freq(state_low, state_high);//高低频同时混频
                break;
            }
            case 1:                     
            {
                state_low = 0;          //低频频率1k
                Mode_Freq(state_low, state_high);
                break;
            }
            case 2:                     
            {
