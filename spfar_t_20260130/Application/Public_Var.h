...
extern volatile unsigned char state_low;                                      // 当前低频工作状态
extern volatile unsigned char state_high;   
...
extern volatile unsigned char state_set_temp;  //暂时变量，用于兼容旧上位机使用。使用新上位机删除这个
//预设置参数
...
#define FPGA_Amp_Addr_low  0x00100001
#define FPGA_Amp_Addr_high 0x00100002
...
extern Uint16 CMD_SetFreq[8];                                                   //控制FPGA数据采集，写入的数据命令
extern Uint16 CMD_SetAmp[6];                                                    //控制FPGA输出幅度
/*************************************************************/
...
#define ADC_REF    3000.f                                                       //DSP ADC 参考电压
...
extern unsigned int  Frame_Length_Spfar;                                          //超远探固定帧数637
...
extern unsigned char state_freq;                                                       //存放当前工作频率 在帧内占1字节
